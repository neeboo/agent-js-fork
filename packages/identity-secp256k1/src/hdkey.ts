...
HDKey.prototype.deriveChild = function (index: number) {
  ...
  const I = hmac.create(sha512, this.chainCode).update(data).digest();
...
HDKey.prototype.wipePrivateData = function () {
  if (this._privateKey) {
    this._privateKey = randomBytes(this._privateKey.length);
  }
  this._privateKey = null;
  return this;
};
...
HDKey.fromMasterSeed = function (seedBuffer: Buffer, versions?: any) {
  const I = hmac.create(sha512, MASTER_SECRET).update(seedBuffer).digest();
  const IL = I.slice(0, 32);
  const IR = I.slice(32);
